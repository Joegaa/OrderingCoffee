Public Class HotBrew

    Const Hot As Double = 45
    Const LargeHot As Double = 55
    ...
    Private Sub AddOrderButton_Click(sender As Object, e As EventArgs) Handles AddOrderButton.Click
        ...
        If LargeSizeBox.Checked = False Then
            ...
            Americano = Format(IcedAme.Text * Hot, "0.00")
            SpanishLat = Format(Spanish.Text * Hot, "0.00")
            JavaLatte = Format(Javalat.Text * Hot, "0.00")
            MochaLatte = Format(MochaLat.Text * Hot, "0.00")
            CappuccinoLat = Format(CappuLat.Text * Hot, "0.00")
            MatchaLatte = Format(MatchaLat.Text * Hot, "0.00")
            Caramel = Format(CaramelMac.Text * Hot, "0.00")
            Chocolate = Format(IcedChoco.Text * Hot, "0.00")
            WhiteMoc = Format(WhiteMocha.Text * Hot, "0.00")
            SaltedCara = Format(SaltedLat.Text * Hot, "0.00")

        ElseIf LargeSizeBox.Checked = True Then

            Americano = Format(IcedAme.Text * LargeHot, "0.00")
            SpanishLat = Format(Spanish.Text * LargeHot, "0.00")
            JavaLatte = Format(Javalat.Text * LargeHot, "0.00")
            MochaLatte = Format(MochaLat.Text * LargeHot, "0.00")
            CappuccinoLat = Format(CappuLat.Text * LargeHot, "0.00")
            MatchaLatte = Format(MatchaLat.Text * LargeHot, "0.00")
            Caramel = Format(CaramelMac.Text * LargeHot, "0.00")
            Chocolate = Format(IcedChoco.Text * LargeHot, "0.00")
            WhiteMoc = Format(WhiteMocha.Text * LargeHot, "0.00")
            SaltedCara = Format(SaltedLat.Text * LargeHot, "0.00")
        ...
        HotBrew = Americano + SpanishLat + JavaLatte + MochaLatte + CappuccinoLat + MatchaLatte + Caramel + Chocolate + WhiteMoc + SaltedCara

        Form1.UpdateTotal(HotBrew)
